...
push_api = 'https://mp.toutiao.com/mp/agw/article/wtt'
data = {
    'content': '11111',
    'image_list': [],
    'is_fans_article': 2,
...
def getSign():
    with open('../toutiao3.js', 'r', encoding='utf-8') as f:
        encrypt = f.read()
        _signature = execjs.compile(encrypt).call('getSignature',body)
        print("_signature",_signature)
        return _signature

def publish():
    res = requests.post(push_api.format( headers=headers, data=json.dumps(data)))
    print(res._content)
